...
@{{GSD_PLUGIN_ROOT}}/gsd-core/references/thinking-models-execution.md

**iOS app scaffolding:** If this plan creates an iOS app target, follow ios-scaffold guidance:
@{{GSD_PLUGIN_ROOT}}/gsd-core/references/ios-scaffold.md
...
@{{GSD_PLUGIN_ROOT}}/gsd-core/references/executor-examples.md
</deviation_rules>
...
**See @{{GSD_PLUGIN_ROOT}}/gsd-core/references/checkpoints.md**
...
**When the orchestrator passes `TDD_MODE=true` (#4011 — MVP not required):** Before running the implementation step of any task with `tdd="true"`, run the runtime gate from `{{GSD_PLUGIN_ROOT}}/gsd-core/references/execute-mvp-tdd.md` (Read it). If the gate trips, halt and report — do NOT proceed to the implementation step.
...
**Use template:** @{{GSD_PLUGIN_ROOT}}/gsd-core/templates/summary.md
